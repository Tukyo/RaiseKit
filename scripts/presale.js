...
async function generateReferralLink() {
    if (window.ethereum) {
        ...
        window.ethereum.request({ method: 'eth_requestAccounts' }).then(async accounts => {
            ...
            // Assign the base URL to the actual referral link
            ...
            if (ensName) {
                ...
                actualReferralLink = `${baseUrl}?ref=${ensName}`;
            } else {
...
function ensContextualClaim() {
    ...
    // Define common syllables or word fragments
    const syllables = ['ba', 'be', 'bi', 'bo', 'bu', 'ca', 'ce', 'ci', 'co', 'cu', 'da', 'de', 'di', 'do', 'du', 'fa', 'fe', 'fi', 'fo', 'fu', 'ga', 'ge', 'gi', 'go', 'gu', 'ha', 'he', 'hi', 'ho', 'hu', 'ja', 'je', 'ji', 'jo', 'ju', 'ka', 'ke', 'ki', 'ko', 'ku', 'la', 'le', 'li', 'lo', 'lu', 'ma', 'me', 'mi', 'mo', 'mu', 'na', 'ne', 'ni', 'no', 'nu', 'pa', 'pe', 'pi', 'po', 'pu', 'ra', 're', 'ri', 'ro', 'ru', 'sa', 'se', 'si', 'so', 'su', 'ta', 'te', 'ti', 'to', 'tu', 'va', 've', 'vi', 'vo', 'vu', 'wa', 'we', 'wi', 'wo', 'wu', 'ya', 'ye', 'yi', 'yo', 'yu', 'za', 'ze', 'zi', 'zo', 'zu'];
    const minLength = 5;  // minimum length of ENS name
    const maxLength = 10; // maximum length of ENS name

    // Helper function to generate random ENS-like names
    function generateRandomEnsName() {
        let randomEnsName = '';
        while (randomEnsName.length < minLength || randomEnsName.length > maxLength) {
            randomEnsName = '';
            while (randomEnsName.length < maxLength) {
                const randomIndex = Math.floor(Math.random() * syllables.length);
                randomEnsName += syllables[randomIndex];
                // Randomly insert a number
                if (Math.random() < 0.3 && randomEnsName.length < maxLength) {
                    randomEnsName += Math.floor(Math.random() * 10);
                }
                if (randomEnsName.length >= minLength && randomEnsName.length <= maxLength) {
                    break;
                }
            }
        }
        return randomEnsName + '.eth';
    }

    setInterval(() => {
        const randomEnsName = generateRandomEnsName();
